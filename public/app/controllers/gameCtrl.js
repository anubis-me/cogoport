...
angular.module('gameController',['gameService'])
    ...
    app.advancedSearch = function(searchByPlatform, searchByGenre, searchByName) {
    // Ensure only to perform advanced search if one of the fields was submitted
    if (searchByPlatform || searchByGenre || searchByName) {
        ...
        if (searchByGenre) {
            $scope.advancedSearchFilter.genre = searchByGenre; // If email keyword was provided, search by email
        }
        if (searchByName) {
